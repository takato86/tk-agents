...
import copy
import numpy as np
from tk_agents.agent.base import BaseAgent
from tk_agents.network.actor_critic import ActorCritic
from tk_agents.utils.mpi import mpi_avg_grad, sync_params


class DDPGAgent(BaseAgent):
    def __init__(self, observation_space, action_space, gamma, actor_lr, critic_lr, polyak, l2_action, clip_return,
                 is_pos_return, logger, device):
        self.gamma = gamma
        self.device = device
        ...
        self.max_act_tensor = torch.Tensor(action_space.high).to(self.device)
